# vet-inventory - Development Guide
...
- **Trigger name**: `trg_vet_inventory_{descriptive_name}`
- **Function name**: `trg_vet_inventory_{descriptive_name}_fn`

Where `vet_inventory` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_vet_inventory_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_vet_inventory_audit_log"
AFTER INSERT OR UPDATE ON "module_vet_inventory_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_vet_inventory_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_vet_inventory_*` triggers when the plugin
